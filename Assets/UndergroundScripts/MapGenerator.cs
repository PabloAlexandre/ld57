...
public class MazeManager {
    ...
    public void generateMaze() {
        while (cellStack.Count > 0) {
            ...
            CellNeighbor[] neighbors = getNeighbors(currentCell);

            if (neighbors.Length > 0) {
                int randIndex = UnityEngine.Random.Range(0, neighbors.Length);
                CellNeighbor neighbor = neighbors[randIndex];
    ...
    public CellNeighbor[] getNeighbors(MapCell cell) {
        int[][] dirs = new int[][] {
            new int[] { 0, -1 }, // up
            new int[] { 1, 0 }, // right
            new int[] { 0, 1 }, // down
